...
export function createLibraryService(libraryRoot) {
  ...
  async function findSourceByOriginalName(originalName) {
    const normalized = path.basename(originalName).trim().toLowerCase();
    const documents = await listAllSourceDocuments();
    return documents.find((document) => path.basename(document.originalName).trim().toLowerCase() === normalized) ?? null;
  }
  ...
  async function archiveSource(sourceId) {
    ...
    const document = { ...existing, deletedAt: new Date().toISOString() };
  ...
  async function restoreSource(sourceId) {
    const existing = await readSourceDocument(sourceId);
    const document = { ...existing, deletedAt: null, restoredAt: new Date().toISOString() };
    await atomicWriteJson(sourceDocumentPathFor(sourceId), document);
    return document;
  }
  ...
  async function beginImport(audioPath) {
    ...
    if (existing) {
      const error = new Error("This recording is already in your library.");
   ...
   * `waveform` already do.
   *
   * Each fragment's `createdAt` (its "uploaded" timestamp for sorting) is
   * preserved from the previous document if that fragment id already
   * existed, rather than trusting the renderer to send it - so renaming or
   * re-slicing a fragment never bumps it to the top of a "latest uploaded"
   * sort.
   */
  async function updateFragments(sourceId, fragments) {
    ...
    const stamped = fragments.map((fragment) => ({
      ...fragment,
      createdAt: existingById.get(fragment.id)?.createdAt ?? fragment.createdAt ?? new Date().toISOString(),
    }));
    const document = { ...existing, fragments: stamped };
    await atomicWriteJson(sourceDocumentPathFor(sourceId), document);
    return document;
  }

  /**
   * Overwrites the (algorithmic) relationships list on an existing source
   * document - the "affinities" this source's fragments have to fragments
   * in other sources. Stored per-source (keyed by the relationship's
   * `source` fragment id) so the whole library's affinity graph survives an
   * app restart the same way fragments and analysis already do.
   */
  async function updateRelationships(sourceId, relationships) {
    if (!Array.isArray(relationships)) throw new Error("relationships must be an array");
    const existing = await readSourceDocument(sourceId);
    const document = { ...existing, relationships };
    await atomicWriteJson(sourceDocumentPathFor(sourceId), document);
    return document;
  }
