...
export function createLibraryService(libraryRoot) {
  ...
  async function findSourceByContentHash(contentHash) {
    const documents = await listAllSourceDocuments();
    return documents.find((document) => document.contentHash === contentHash) ?? null;
  }

  /**
   * Marks a source as removed from the library without deleting its on-disk
   * folder. Re-importing the same audio (matched by content hash) restores it.
   */
   ...
   *
   * If the file matches a soft-deleted source by content hash, that source is
   * restored and returned instead of creating a new directory.
   *
