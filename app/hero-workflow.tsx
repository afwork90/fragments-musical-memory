...
export function CombineWorkspace({ anchor,candidates,fragments,statuses,onClose,onEdit,onExport,onSave,onReject,onAuditioned }: {
  ...
  const stop=() => { audios.current.forEach((audio) => { audio.pause();audio.currentTime=0; });audios.current=[];timers.current.forEach(window.clearTimeout);timers.current=[];setPlaying(""); };
  useEffect(() => () => stop(),[]);
  ...
  const play=(mode:"A" | "B" | "A→B" | "B→A" | "Together") => {
    ...
    const safe=(audio:HTMLAudioElement) => audio.play().catch(() => setPlaying(""));
    if (mode === "A") safe(a);if (mode === "B") safe(b);if (mode === "Together") { safe(a);safe(b); }
    if (mode === "A→B") { safe(a);timers.current.push(window.setTimeout(() => { a.pause();safe(b); },2600)); }
    if (mode === "B→A") { safe(b);timers.current.push(window.setTimeout(() => { b.pause();safe(a); },2600)); }
  };
  ...
  return <section className="combine-workspace" aria-label="Combine workspace">
    ...
    <div className="candidate-strip" aria-label="Top candidates">{candidates.slice(0,3).map((item) => { const fragment=fragments.find((entry) => entry.id === item.otherId)!;const itemStatus=statuses[item.id];return <button className={item.id === relationship.id ? "active" : ""} onClick={() => chooseCandidate(item.id)} key={item.id}><RealWave values={fragment.waveform}/><span><b>{fragment.name}</b><small>{item.score}% · {fragment.key} · {fragment.bpm} BPM</small></span>{itemStatus && itemStatus !== "suggested" && <em className={`relationship-badge ${itemStatus}`}>{itemStatus}</em>}</button>;})}</div>
    <div className="combine-grid">
      <div className="combine-stage">
        <div className="combine-track"><div className="track-index">A</div><div><span className="eyebrow">Anchor</span><h2>{anchor.name}</h2><RealWave values={anchor.waveform} active={playing !== ""}/></div><div className="track-mix"><button className={loop.a ? "active" : ""} onClick={() => setLoop((current) => ({ ...current,a:!current.a }))}>Loop</button><button className={mute.a ? "active" : ""} onClick={() => setMute((current) => ({ ...current,a:!current.a }))}>{mute.a ? "Muted" : "Mute"}</button><input aria-label="Anchor volume" type="range" min="0" max="100" value={volume.a} onChange={(event) => setVolume((current) => ({ ...current,a:Number(event.target.value) }))}/></div></div>
        <div className="combine-track candidate"><div className="track-index">B</div><div><span className="eyebrow">Candidate · {transform.transformed ? "Transformed" : "Original"}</span><h2>{candidate.name}</h2><RealWave values={[...candidate.waveform].reverse()} active={playing !== ""}/></div><div className="track-mix"><button className={loop.b ? "active" : ""} onClick={() => setLoop((current) => ({ ...current,b:!current.b }))}>Loop</button><button className={mute.b ? "active" : ""} onClick={() => setMute((current) => ({ ...current,b:!current.b }))}>{mute.b ? "Muted" : "Mute"}</button><input aria-label="Candidate volume" type="range" min="0" max="100" value={volume.b} onChange={(event) => setVolume((current) => ({ ...current,b:Number(event.target.value) }))}/></div></div>
        ...
        <div className="relationship-reason"><span>Why it connects</span><p>{relationship.reason}</p><div><b>{relationship.score}% fit</b>{relationship.transform?.labels.map((label) => <i key={label}>{label}</i>)}{status && <em className={`relationship-badge ${status}`}>{status}</em>}</div></div>
      </div>
    ...
    <footer className="combine-actions"><button onClick={() => { const next=candidates.find((item) => item.id !== relationship.id);if (next) chooseCandidate(next.id);onReject(relationship); }}>Reject</button><button onClick={() => onEdit(relationship)}>Edit source</button><button onClick={() => onExport(relationship)}>Export</button><button className="primary-button" onClick={() => onSave(relationship)}>Save combination</button></footer>
  </section>;
}
...
export function CorrectionOverlay({ candidate,source,surrounding,score,onCancel,onApply,onKeep,onDrop }: { candidate:Fragment;source:SourceFile;surrounding:Fragment[];score:number;onCancel:()=>void;onApply:(patch:Partial<Fragment>)=>void;onKeep:()=>void;onDrop:()=>void }) {
  ...
  return <div className="correction-layer"><section className="correction-editor" role="dialog" aria-modal="true" aria-label="Edit source boundaries">
    <header><div><span className="eyebrow">Fragmentation</span><h2>{source.name}</h2><p>Editing {candidate.name} while preserving source context</p></div><button className="modal-close" onClick={onCancel} aria-label="Discard boundary changes">×</button></header>
    <div className="correction-timeline">
      <div className="correction-lanes">{surrounding.map((fragment,index) => <i key={fragment.id} style={{ top:`${index * 24}px`,left:`${fragment.start / source.duration * 100}%`,width:`${(fragment.end - fragment.start) / source.duration * 100}%`,background:COLORS[index % COLORS.length] }}><span>{fragment.name}</span></i>)}</div>
      <div className="correction-wave"><RealWave values={source.waveform}/>{surrounding.map((fragment,index) => <i key={fragment.id} style={{ left:`${fragment.start / source.duration * 100}%`,width:`${(fragment.end - fragment.start) / source.duration * 100}%`,borderColor:COLORS[index % COLORS.length] }}/>)}</div>
